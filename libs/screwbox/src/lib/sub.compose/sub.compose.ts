 ...
 */
export function defaultKeys(object: any, keys: string[], value?: any): any {
  const holder = {} as any;
  keys.forEach(e => {
    if (!object[e]) {
      holder[e] = value || null;
 ...
 */
export function getProp(value: any, key: string, defValue = null) {
...
export function reader(file: File, method?: string): Promise<string> {
  ...
  return new Promise((resolve, reject) => {
    fReader.addEventListener(
      "load",
      () => {
        return resolve((fReader as any).result);
      },
      false
    );
    fReader.onerror = err => reject(err);
    if (file) {
      (fReader as any)[method ? method : "readAsDataURL"](file);
    }
  });
}
