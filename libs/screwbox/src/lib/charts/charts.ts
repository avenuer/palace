...
import { errorInvalidArray } from '../sub.compose/sub.compose';
 ...
 */
export function single<T>(data: T[], config: ChartMap<any>): Single[] {
 ...
 */
export function series<T>(data: T[], config: SeriesMap<T>) {
  if (typeof config !== 'object') {
...
export function quaitave<T>(
  ...
  return Object.keys(_data).map((e, i) => {
    const _obj: any = _data[e];
    const _array = _obj.map((obj: any) => {
      return {
        name: obj[name],
        value: obj[value],
      };
